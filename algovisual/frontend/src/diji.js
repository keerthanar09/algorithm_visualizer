...
function visualizeDijkstra(source) {
  ...
  const interval = setInterval(() => {
    if (priorityQueue.isEmpty()) {
      ...
      let resultText = 'Shortest distances from Node ${source}:<br>';
      for (let i = 0; i < NODE_COUNT; i++) {
        if (i !== source) {
          resultText += `Node ${source} -> Node ${i} --> Distance: ${distances[i] === Infinity ? 'Infinity' : distances[i]}<br>`;
        }
      }
...
startButton.addEventListener("click", () => {
  const source = parseInt(sourceInput.value, 10);

  if (isNaN(source) || source < 0 || source >= NODE_COUNT) {
    alert("Please enter a valid source node between 0 and 5.");
    return;
  }
  generateRandomGraph();
  drawGraph();
  visualizeDijkstra(source);
});